    ...
    *,
    ...
    if segments is not None:
        ...
        <div class="card">
          ...
          <i>без</i> фильтра «Заключено соглашение».</p>
          {seg_html}
        </div>
        """
...
# внутри:
#   I = РезультатПроверки=1 & соглашение=1 & выплата_по_модели=1
#   e_fee = p_fu*100_000 + p_court*15_000
#   expected_psr = precision * (sum(ОД_I * k) + n_I * e_fee)
#   cost = sum(СуммаПлатежа_I)
...
<div class="page">
  ...
  <h2>1. Что считаем</h2>
  <div class="card">
    <p><b>Интервенция I</b> — кейсы, где модель сказала платить, заключили соглашение
    и выплата по модели прошла:</p>
    <div class="formula">I: РезультатПроверки = 1
   ∧ Заключено соглашение = 1
   ∧ Выплата по модели в Инциденте = 1</div>
    <p>На этих кейсах оцениваем, сколько ПСР+взносов мы <i>ожидаемо избежали</i>,
    и вычитаем фактическую доплату клиенту.</p>
  </div>
...
expected_psr = precision × Σ_{{i ∈ I}} (ОД_i × k + e_fee)
             = precision × ( Σ_I ОД × k  +  n_I × e_fee )

cost = Σ_{{i ∈ I}} СуммаПлатежа_i
...
net  = expected_psr − cost</div>
    <ul>
      ...
      <li><b>cost</b> — факт платежа (<code>{escape(effect.cost_column)}</code>), не ОД.</li>

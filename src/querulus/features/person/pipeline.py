...
from querulus.features.person.history_pretensions import (
    _prep_pretensions,
    add_person_pretension_history,
)
...
def run_person_features(
    ...
    *,
    ...
    pret = _prep_pretensions(
        load_pretensions_base(paths, conn, use_sql=use_sql, save_checkpoint=save_checkpoint)
    )
    try:
        pret_3 = _prep_pretensions(
            load_pretensions_penalty_surcharge(

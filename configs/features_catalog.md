...
|------|----------|----------------|
| `TARGET` | Бинарный ПСР (было взыскание/выплата по претензиям) | `build_targets`: 1 если `Сумма_выплат_по_претензиям + Сумма_взыскано_по_ФУ + Суммы_взыскано_по_иску > 0` |
| `TARGET_SEV` | Сумма тяжести (взыскание + доплаты) | `build_targets`: последний ненулевой `RECOVERED*` по инстанциям иска + `SurchargeValue_cumsum_by_incident` + `UTSSurchargeValue_cumsum_by_incident` |

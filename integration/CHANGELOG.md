# Changelog — Querulus scoring service

Версия API — поле `oisuu_responce["version"]` в `main.py` и `GET /api/health`.
...
- **Документация в коде** — модульные докстринги, комментарии к секциям в `main.py`, `unit_tests.py`, `integration_tests.py`, `test_instruction.md`.
